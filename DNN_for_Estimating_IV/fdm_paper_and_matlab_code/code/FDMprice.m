function p=FDMprice(sigma0,alpha,beta,rho,strike,tenor)
forward=1.0;nu=alpha;alpha=sigma0;T=tenor;
timesteps=tenor * 5;nd=4;N=80;
p=priceCallTransformedSABRDensity(strike, alpha, beta, nu, rho, forward, T, N,timesteps,nd);
end

function p = priceCallTransformedSABRDensity(strike, alpha, beta, nu, rho, forward, T, N,timesteps,nd)
[P, PL, PR, zm, zmin, zmax, h] = makeTransformedSABRDensityLawsonSwayne(alpha, beta, nu, rho, forward, T, N, timesteps, nd);
...
F = H(forward, beta, Y(alpha,nu,rho,z)); 
end
function y = yOfStrike(strike , forward , beta)
y = (sign(strike)*abs(strike)^(1-beta)-sign(forward)*abs(forward)^(1-beta))/(1- beta);
end
